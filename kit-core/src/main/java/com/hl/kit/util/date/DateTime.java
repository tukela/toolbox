...
public class DateTime {
	 ...
	 */
	public static long addMinutes(final int minute) {
		long millis = System.currentTimeMillis();
		millis = millis + (minute * 60L * 1000L);
		return getLongTime(millis);
	}
	 ...
	 * @return 当前时间
	 */
	public synchronized static Long getLongTime() {
		Date date = new Date();
		String format = GET_INT_TIME_FORMAT.format(date);
		long parseLong = Long.parseLong(format);
		return parseLong;
	}
	
	public static synchronized Long getLongTime(final long millis) {
		Date date = new Date();
		if (millis != 0) {
			date.setTime(millis);
		}
	 ...
	 * 将long时间转化为正常格式
	 */
	public String LongTimeTOString(Long value){
		String result=value+"";
		if (result.length() == 8)
		{
			String time = value + "";
		...
		else if (result.length() == 10)
		{
			...
			result = year + "-" + month + "-" + day + " " + hour + ":00:00";
		}
